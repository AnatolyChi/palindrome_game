...
public interface IElementNumberAssigner {
  ...
  /**
   * Метод выставляет номера {@link IElement#setupNumber(int)} для элементов коллекции {@code elements} в порядке
   * следования элементов в коллекции.
   * <p>
   ...
   * </li>
   * <li>
   * <p>метод устойчив к передаче в него в качестве параметра {@link java.util.Collections#unmodifiableList(List)} и
   * любой другой реализации immutable-list,</p>
   * </li>
   * <li>
   * <p>метод должен работать за «приемлемое» время ({@link IElement#setupNumber(int)} - трудоемкая операция и
   * пользоваться ей надо рационально)</p>
   * </li>
   * </ul>
   *
